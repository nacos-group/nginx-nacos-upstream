...
static ngx_int_t ngx_aux_nacos_proc_handler(ngx_cycle_t *cycle, ngx_aux_proc_t *p);
...
static ngx_int_t ngx_nacos_aux_get_addr(ngx_peer_connection_t *pc, void *data);

static void ngx_nacos_aux_free_addr(ngx_peer_connection_t *pc, void *data, ngx_uint_t state);

typedef ngx_int_t (*ngx_nacos_connect_handler_t)(ngx_aux_nacos_conn_t *nc, ngx_event_t *ev);

static ngx_int_t ngx_nacos_write_sub_request(ngx_aux_nacos_conn_t *nc, ngx_event_t *ev);

static ngx_int_t ngx_nacos_read_sub_resp(ngx_aux_nacos_conn_t *nc, ngx_event_t *ev);

static void ngx_nacos_close_http_connection(ngx_aux_nacos_conn_t *nc, ngx_uint_t state);

static ngx_int_t ngx_nacos_create_req_buf(ngx_aux_nacos_conn_t *nc);

static ngx_connection_t *ngx_nacos_open_udp();

static ngx_nacos_key_t *ngx_nacos_hash_find_key(u_char *k, ngx_log_t *log, ngx_str_t *addr_txt);
...
static ngx_aux_proc_t aux_proc = {
        ngx_string("nacos"),
        NULL,
...
static ngx_nacos_key_t *ngx_nacos_hash_find_key(u_char *k, ngx_log_t *log, ngx_str_t *addr_txt) {
    ...
    if (!valid) {
        ngx_log_error(NGX_LOG_WARN, log, 0, "receive udp msg from %V data name \"%s\" is invalid",
                      addr_txt, k);
        return NULL;
    }

    return ngx_hash_find(aux_ctx.ncf->key_hash, hash, (u_char *) k, p - k);
}
...
static ngx_int_t ngx_aux_nacos_proc_handler(ngx_cycle_t *cycle, ngx_aux_proc_t *p) {
    ...
    switch (aux_ctx.ncf->udp_addr.sockaddr->sa_family) {
#if (NGX_HAVE_INET6)
        case AF_INET6:
            aux_ctx.udp_remote_addr_text_len = NGX_INET6_ADDRSTRLEN;
            break;
#endif
#if (NGX_HAVE_UNIX_DOMAIN)
        case AF_UNIX:
            aux_ctx.udp_remote_addr_text_len = NGX_UNIX_ADDRSTRLEN;
            break;
#endif
        case AF_INET:
            aux_ctx.udp_remote_addr_text_len = NGX_INET_ADDRSTRLEN;
            break;
        default:
            aux_ctx.udp_remote_addr_text_len = NGX_SOCKADDR_STRLEN;
            break;
    }
    aux_ctx.udp_remote_addr_text = ngx_palloc(pool, aux_ctx.udp_remote_addr_text_len);
    if (aux_ctx.udp_remote_addr_text == NULL) {
        ngx_close_connection(c);
...
static void ngx_nacos_http_event_handler(ngx_event_t *ev) {
    ...
    ngx_nacos_close_http_connection(nc, rc == NGX_DONE ? NC_TIRED : NC_ERROR);
    aux_ctx.nc = NULL;
    if (aux_ctx.err_times >= aux_ctx.ncf->server_list.nelts) {
        ngx_add_timer(aux_ctx.uc->write, 10000);
    } else {
        ngx_add_timer(aux_ctx.uc->write, 100);
    }
}
...
static ngx_aux_nacos_conn_t *nax_nacos_create_http_connection() {
    ...
    nc->peer.log = aux_ctx.ncf->error_log;
    ...
    if (rc == NGX_ERROR) {
        if (nc->peer.name) {
            ngx_log_error(NGX_LOG_WARN, nc->peer.log, 0, "http connection connect to %V error",
                          nc->peer.name);
        }
        if (nc->peer.sockaddr) {
    ...
    connect_failed:
    ngx_log_error(NGX_LOG_WARN, nc->peer.log, 0, "create http connection error after try %d", try);
    ngx_destroy_pool(pool);
    return NULL;
}
...
static ngx_int_t ngx_nacos_aux_get_addr(ngx_peer_connection_t *pc, void *data) {
    ngx_addr_t *t;
    ngx_uint_t i, n;
    t = aux_ctx.ncf->server_list.elts;
    i = aux_ctx.ncf->cur_srv_index;
    n = aux_ctx.ncf->server_list.nelts;
    if (i >= n) {
        i %= n;
    }
...
static void ngx_nacos_aux_free_addr(ngx_peer_connection_t *pc, void *data, ngx_uint_t state) {
    ngx_uint_t i, n;
    i = aux_ctx.ncf->cur_srv_index;
    n = (ngx_int_t) aux_ctx.ncf->server_list.nelts;
    ...
    ngx_log_error(NGX_ERROR_INFO, aux_ctx.ncf->error_log, 0, "use next addr because of error");
}
...
static ngx_int_t ngx_nacos_write_sub_request(ngx_aux_nacos_conn_t *nc, ngx_event_t *ev) {
    ...
    if (nc->stat == connecting) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection connect successfully");
        nc->stat = prepare_req;
    }

    if (nc->stat == prepare_req) {
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "nacos http connection prepare request");
    ...
    if (nc->stat == writing_req) {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection write request successfully");
    }
...
static ngx_int_t ngx_nacos_handle_sub_json(yajl_val json, ngx_nacos_key_t *key,
                                           ngx_pool_t *pool, char *buf, size_t buf_len) {
    ...
    if (adr != NULL) {
        ...
        cache.adr = adr;
        return ngx_nacos_write_disk_data(aux_ctx.ncf, &cache);
    }
    return NGX_DECLINED;
}
...
static ngx_int_t ngx_nacos_read_sub_resp(ngx_aux_nacos_conn_t *nc, ngx_event_t *ev) {
    ...
    if (nc->stat == waiting_resp) {
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection start reading response");
    ...
    if (nc->stat == reading_resp) {
        for (;;) {
            ...
            rd = c->recv(c, nc->parser.buf + nc->parser.limit, NC_BUF_SIZE - nc->parser.limit);
            if (rd >= 0) {
                nc->parser.limit += rd;
                ...
                if (rc == NGX_ERROR) {
                    ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection protocol error");
                    return NGX_ERROR;
                } else if (rc == NGX_OK) {
                ...
                if (ev->eof) {
                    // close premature
                    ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection close abruptly");
                    return NGX_ERROR;
                }
            } else if (rd == NGX_AGAIN) {
                return NGX_OK;
            } else {
                ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection encounter socket error");
                // socket error
                return NGX_ERROR;
            }
        }
    ...
    request_complete:
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "http connection read response successfully,status=%d",
                   nc->parser.status);
    c->requests++;
    ...
    if (nc->parser.status == 200) {
        ...
        if (aux_ctx.heart_time == 0) {
            if (++aux_ctx.sub_keys >= aux_ctx.ncf->keys.nelts) {
                aux_ctx.heart_time = ngx_current_msec;
            }
        } else if (ngx_current_msec - aux_ctx.heart_time >= 120000) {// 120s
            aux_ctx.sub_keys = 0;
            aux_ctx.heart_time = 0;
        }
        if (aux_ctx.heart_time) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "send next nacos sub request after %ul", next_req_time);
...
static ngx_int_t ngx_nacos_create_req_buf(ngx_aux_nacos_conn_t *nc) {
    ...
    len = aux_ctx.ncf->keys.nelts;
    if (idx >= len) {
        aux_ctx.key_idx = 0;
        idx = 0;
    }

    nc->buf_len = ngx_sprintf(nc->buf, NACOS_REQ_FMT,
                              &key[idx].group,
                              &key[idx].data_id,
                              &aux_ctx.ncf->udp_port,
                              &aux_ctx.ncf->udp_ip,
                              nc->peer.name) - nc->buf;
...
static ngx_connection_t *ngx_nacos_open_udp() {
    ...
    if (bind(s, aux_ctx.ncf->udp_addr.sockaddr, aux_ctx.ncf->udp_addr.socklen) == -1) {
        ngx_log_error(NGX_LOG_CRIT, log, ngx_socket_errno,
                      "bind(%V) failed", &aux_ctx.ncf->udp_addr.name);
...
static void ngx_nacos_udp_write_handler(ngx_event_t *ev) {
    ...
    nc = aux_ctx.nc;
    if (nc != NULL) {
        ngx_log_error(NGX_LOG_WARN, c->log, 0, "event write but nacos http connection exists");
        aux_ctx.nc = NULL;
        ngx_nacos_close_http_connection(nc, NC_TIRED);
    }

    nc = nax_nacos_create_http_connection();
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, c->log, 0, "open http connection success");
    aux_ctx.nc = nc;

}

static void ngx_nacos_close_http_connection(ngx_aux_nacos_conn_t *nc, ngx_uint_t state) {
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, nc->peer.log, 0, "close http connection");
...
static void ngx_nacos_udp_read_handler(ngx_event_t *ev) {
    ...
    n = recvfrom(c->fd, buffer, sizeof(buffer), 0, (struct sockaddr *) &sa, &socklen);
    ...
    if (socklen == 0) {
        ...
        sa.sockaddr.sa_family = aux_ctx.ncf->udp_addr.sockaddr->sa_family;
    }
    addr_text.data = aux_ctx.udp_remote_addr_text;
    addr_text.len = aux_ctx.udp_remote_addr_text_len;
    addr_text.len = ngx_sock_ntop(&sa.sockaddr, socklen,
                                  addr_text.data,
                                  addr_text.len, 1);
    json = yajl_tree_parse_with_len((const char *) buffer, n);
    if (json == NULL) {
        ngx_log_error(NGX_LOG_WARN, c->log, 0, "receive udp msg from %V not valid json",
                      &addr_text);
        return;
    }
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "udp received a json from %V", &addr_text);
    ...
    if (ngx_strncmp(type_str, "dom", 3) == 0) {
        ...
        key = ngx_nacos_hash_find_key((u_char *) YAJL_GET_STRING(name), c->log, &addr_text);
        if (key != NULL) {
            temp = ngx_create_pool(512, c->log);
            if (temp == NULL) {
                goto end;
            }
            ngx_log_error(NGX_LOG_INFO, c->log, 0, "nacos udp dom %s is received from:%V", YAJL_GET_STRING(name),
                          &addr_text);
            ngx_nacos_handle_sub_json(data, key, temp, (char *) buffer, sizeof(buffer));
        } else {
            ngx_log_error(NGX_LOG_WARN, c->log, 0, "nacos udp dom  %s is unknown from:%V", YAJL_GET_STRING(name),
                          &addr_text);
        }
    }
    ...
    if (dom_update) {
        resp_len = ngx_snprintf(buffer, sizeof(buffer), NACOS_DOM_RESP_FMT, YAJL_GET_NUMBER(ref)) - buffer;
    } else {
        resp_len = ngx_snprintf(buffer, sizeof(buffer), NACOS_UNKNOWN_RESP_FMT, YAJL_GET_NUMBER(ref)) - buffer;
    }

    n = sendto(c->fd, buffer, resp_len, 0, &sa.sockaddr, socklen);
    if (n != (ssize_t) resp_len) {
        ngx_log_error(NGX_LOG_WARN, c->log, ngx_socket_errno, "udp resp not send successfully???");
    }
